const DomatorApp = (() => {
  ...
  const bindEvents = () => {
    document
      .getElementById("addDomatorProductBtn")
      .addEventListener("click", addProduct);
    document
      .getElementById("domatorClearBtn")
      .addEventListener("click", clearForm);
    document
      .getElementById("domatorSaveBtn")
      .addEventListener("click", saveToStorage);
    document
      .getElementById("loadDomatorTemplateBtn")
      .addEventListener("click", loadTemplate);
    document
      .getElementById("domatorExportEmailBtn")
      .addEventListener("click", exportToEmail);
    document
      .getElementById("domatorExportPdfBtn")
      .addEventListener("click", exportToPdf);
    document
      .getElementById("domatorCopyHtmlBtn")
  ...
  const updateStats = () => {
    ...
    statusEl.textContent =
      clientName && totalQty > 0 ? "✅ Gotowe" : "📝 Edycja";
  ...
  const saveToStorage = async () => {
    ...
    await StorageSystem.db.set(StorageSystem.db.STORES.domator, {
      id: "domatorOrder",
      ...data,
    });
  };
  ...
  const loadFromStorage = async () => {
    const data = await StorageSystem.db.get(
      StorageSystem.db.STORES.domator,
      "domatorOrder",
    );
    if (data) {
      if (data.client) {
        for (const key in data.client) {
          const el = document.getElementById(
            `domatorClient${key.charAt(0).toUpperCase() + key.slice(1)}`,
          );
  ...
  const loadTemplate = () => {
    ...
    products = [
      {
        id: 1,
        sku: "DIABLO-X-RAY-2.0",
        ean: "5902560334231",
        name: "Fotel gamingowy Diablo X-Ray 2.0 Normal Size",
        qty: 2,
        netto: 899.0,
        brutto: 1105.77,
      },
      {
        id: 2,
        sku: "DIABLO-V-COMMANDER",
        ean: "5902560334248",
        name: "Fotel biurowy Diablo V-Commander",
        qty: 1,
        netto: 1299.0,
        brutto: 1597.77,
      },
  ...
  const generateHtmlOrder = (data) => {
    ...
    let totalBrutto = data.products.reduce(
      (sum, p) => sum + p.brutto * p.qty,
      0,
    );
