...
const Dashboard = (() => {
  ...
  const _fetchAndProcessData = async () => {
    const offers = await StorageSystem.db.getAll(
      StorageSystem.db.STORES.offers,
    );
    const monthlyData = {};

    offers.forEach((offer) => {
      const month = new Date(offer.timestamp).toLocaleString("default", {
        month: "long",
      });
      ...
      monthlyData[month].totalValue += offer.products.reduce(
