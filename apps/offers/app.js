const OfferGenerator = (() => {
  ...
  const setupEventListeners = () => {
    ...
    document
      .getElementById("saveOfferBtn")
      ?.addEventListener("click", saveOffer);
    document
      .getElementById("loadOfferBtn")
      ?.addEventListener("click", loadOffer);

    document
      .getElementById("clearFormBtn")
      ?.addEventListener("click", async () => {
        if (
          await UI.Feedback.confirm("Czy na pewno chcesz wyczyścić formularz?")
        ) {
          products = [];
          productImages = {};
          updateProductView();
  ...
  function addProduct(productData) {
    const command = new ProductCommand(
      "add",
      productData,
      products,
      productImages,
  ...
  function duplicateProduct(productId) {
    const command = new DuplicateProductCommand(
      productId,
      products,
      productImages,
    );
  ...
  function getDragAfterElement(container, y) {
    const draggableElements = [
      ...container.querySelectorAll(".product-card:not(.dragging)"),
    ];
  ...
  function updateSummary() {
    ...
    products.forEach((productId) => {
      ...
      const qty =
        parseFloat(document.getElementById(`productQty-${productId}`)?.value) ||
        0;
      const price =
        parseFloat(
          document.getElementById(`productPrice-${productId}`)?.value,
        ) || 0;
      const discount =
        parseFloat(
          document.getElementById(`productDiscount-${productId}`)?.value,
        ) || 0;
    ...
    if (validIdx === 1) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="text-align: center; color: #666; padding: 2rem;">Brak produktów</td></tr>';
    }
  ...
  async function generatePDF() {
    ...
    try {
      ...
      showNotification(
        "Sukces",
        "PDF został pomyślnie wygenerowany!",
        "success",
      );
    } catch (error) {
  ...
  async function saveOffer() {
    ...
    const offerData = {
      id:
        document.getElementById("offerNumber")?.value || `offer_${Date.now()}`,
      ...
      products: products.map((id) => ({
        ...
        price: document.getElementById(`productPrice-${id}`)?.value || "0",
        discount:
          document.getElementById(`productDiscount-${id}`)?.value || "0",
        desc: document.getElementById(`productDesc-${id}`)?.value || "",
        image: productImages[id] || null,
      })),
    ...
    try {
      ...
      showNotification(
        "Zapisano!",
        `Oferta ${offerData.id} została zapisana.`,
        "success",
      );
    } catch (error) {
  ...
  async function loadOffer() {
    ...
    try {
      const allOffers = await StorageSystem.db.getAll(
        StorageSystem.db.STORES.offers,
      );
      ...
      if (profileOffers.length === 0) {
        showNotification(
          "Informacja",
          "Brak zapisanych ofert dla tego profilu.",
          "info",
        );
        return;
      }

      const listHTML = profileOffers
        .map(
          (offer) => `
      ...
      showNotification(
        "Błąd wczytywania",
        "Nie udało się wczytać historii ofert.",
        "error",
      );
    }
  }
  ...
  async function loadOfferFromHistory(offerId) {
    try {
      const offer = await StorageSystem.db.get(
        StorageSystem.db.STORES.offers,
        offerId,
      );
      if (!offer) {
      ...
      showNotification(
        "Błąd",
        "Nie udało się wczytać wybranej oferty.",
        "error",
      );
    }
  }
  ...
  function generateOfferNumber() {
    ...
    if (offerNumberInput)
      offerNumberInput.value = `OF/${year}/${month}/${day}/${random}`;
  }
