...
const PDFManager = (() => {
  ...
  const getOptimalLayout = (productCount, orientation) => {
    ...
    return {
      ...
      pagesNeeded,
      lastPageProducts:
        productCount % layout.productsPerPage || layout.productsPerPage,
    };
  };
  ...
  const estimatePageHeight = (products, orientation) => {
    ...
    const productsHeight = products.reduce(
      (sum, p) => sum + calculateProductHeight(p.desc, orientation),
      0,
    );
  ...
  const renderPDFPage = async (pdf, pageWidth, pageHeight, options) => {
    ...
    tempContainer.style.width =
      (orientation === "portrait" ? 794 : 1123) + "px";
    tempContainer.style.minHeight =
      (orientation === "portrait" ? 1123 : 794) + "px";
      ...
      rows += `
                <tr>
                    ...
                    <td style="border: 1px solid #e5e7eb; padding: 8px;">
                        ...
                        ${
                          product.desc
                            ? `<div style="font-size: 9px; color: #4b5563; margin-top: 4px; line-height: 1.3;">
                            ${product.desc
                              .split("\n")
  ...
  const setMetadata = (pdf, offerData, seller) => {
    const buyerName =
      offerData.buyer && offerData.buyer.name
        ? ` dla ${offerData.buyer.name}`
        : "";
    pdf.setProperties({
