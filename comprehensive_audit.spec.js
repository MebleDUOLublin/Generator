...
test.describe("Pesteczka OS Comprehensive System Audit", () => {
  ...
  test("Offer Generator App - Full Workflow", async ({ page }) => {
    ...
    const productNameInput = productList.locator(
      '.product-card input[placeholder="Nazwa produktu"]',
    );
    ...
    await expect(productList.locator(".product-card")).toHaveCount(1);
    await expect(
      productList.locator('input[value="Test Product"]'),
    ).toBeVisible();
    ...
    // 6. Close the window
    await offersWindow
      .locator('.window-control-btn[data-action="close"]')
      .click();
  ...
  test("Settings App - Save and Verify", async ({ page }) => {
    ...
    await page.dblclick('[data-window="settings"]');
    await expect(settingsWindow).toBeVisible();
    await expect(settingsWindow.locator("#profileFullName")).toHaveValue(
      newFullName,
    );
  ...
  test("Core UI - Window Management and Logout", async ({ page }) => {
    ...
    await offersWindow
      .locator('.window-control-btn[data-action="minimize"]')
      .click();
    await expect(offersWindow).not.toBeVisible();
