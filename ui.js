...
function _createProductCard(productId) {
  ...
  const createFormGroup = (label, input) =>
    createEl("div", { className: "form-group" }, [
      createEl("label", { className: "form-label", textContent: label }),
      input,
    ]);
  const createInput = (id, type, value, oninput) =>
    createEl("input", { id, type, value, oninput, className: "form-input" });
  const createButton = (text, onclick, className) =>
    createEl("button", { textContent: text, onclick, className });
  ...
  const imageZone = createEl("div", { className: "product-image-zone" }, [
    createEl("div", {
      id: `productImagePreview-${productId}`,
    ...
    createEl("input", {
      ...
      accept: "image/*",
      style: "display:none",
      onchange: (e) => uploadProductImage(productId, e),
    }),
  ]);
  imageZone.onclick = () =>
    document.getElementById(`productImage-${productId}`).click();

  const productDetails = createEl("div", { className: "product-details" }, [
    createFormGroup(
      "Nazwa produktu",
      createInput(`productName-${productId}`, "text", "", updateSummary),
    ),
    createEl("div", { className: "form-grid-inner" }, [
      createFormGroup(
        "Kod",
        createInput(`productCode-${productId}`, "text", "", null),
      ),
      createFormGroup(
        "Ilość",
        createInput(`productQty-${productId}`, "number", 1, updateSummary),
      ),
      createFormGroup(
        "Cena netto",
        createInput(`productPrice-${productId}`, "number", 0, updateSummary),
      ),
      createFormGroup(
        "Rabat (%)",
        createInput(`productDiscount-${productId}`, "number", 0, updateSummary),
      ),
    ]),
  ]);
  ...
  productCard.append(
    ...
    createEl("div", { className: "product-content-wrapper" }, [
      imageZone,
      productDetails,
    ]),
    createFormGroup(
...
class DuplicateProductCommand {
  ...
  execute() {
    const originalData = {
      name:
        document.getElementById(`productName-${this.originalProductId}`)
          ?.value || "",
      code:
        document.getElementById(`productCode-${this.originalProductId}`)
          ?.value || "",
      qty:
        document.getElementById(`productQty-${this.originalProductId}`)
          ?.value || "1",
      price:
        document.getElementById(`productPrice-${this.originalProductId}`)
          ?.value || "0",
      discount:
        document.getElementById(`productDiscount-${this.originalProductId}`)
          ?.value || "0",
      desc:
        document.getElementById(`productDesc-${this.originalProductId}`)
          ?.value || "",
    ...
    document.getElementById(`productName-${this.newProductId}`).value =
      originalData.name + " (Kopia)";
    document.getElementById(`productCode-${this.newProductId}`).value =
      originalData.code;
    document.getElementById(`productQty-${this.newProductId}`).value =
      originalData.qty;
    document.getElementById(`productPrice-${this.newProductId}`).value =
      originalData.price;
    document.getElementById(`productDiscount-${this.newProductId}`).value =
      originalData.discount;
    document.getElementById(`productDesc-${this.newProductId}`).value =
      originalData.desc;
      ...
      debounceTimers.set(
        ...
        setTimeout(() => {
          if (enableHistory && newValue !== previousValue) {
            const command = new FieldChangeCommand(
              fieldId,
              previousValue,
              newValue,
            );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  ReactiveForm.createField("offerNumber", {
    onValidate: (value) =>
      value.trim() ? null : "Numer oferty jest wymagany.",
  });
  ReactiveForm.createField("buyerName", {
    onValidate: (value) =>
      value.trim() ? null : "Nazwa nabywcy jest wymagana.",
  });
    ...
    closeAppBtn.addEventListener("click", async () => {
      ...
      if (confirmed) {
        try {
          ...
          UIFeedback.toast(
            "Serwer nie odpowiada. Zamknij okno ręcznie.",
            "error",
          );
        }
      }
