  ...
  const initDB = () => {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (e) => {
        ...
        console.log(
          `📦 Upgrading database from version ${e.oldVersion} to ${e.newVersion}`,
        );
...
const ProfileManager = (() => {
  ...
  const initDefaultProfiles = async () => {
    try {
      ...
      if (data && data.profiles) {
        const profilePromises = Object.values(data.profiles)
          .map((p) => {
            if (!p.key) {
  ...
  const saveProfile = async (profileData) => {
    ...
    const { isValid, errors } = ValidatorModule.validate(
      profileData,
      validationSchema,
    );
    if (!isValid) {
