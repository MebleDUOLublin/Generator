...
const PluginLoader = {
    ...
    async discoverPlugins() {
        try {
            ...
            console.log("Discovered plugin manifests:", pluginManifestPaths);
            ...
            for (const path of pluginManifestPaths) {
                try {
                    ...
                    if (!manifest.id || !manifest.name || !manifest.entrypoints) {
                        throw new Error(`Invalid manifest file at ${path}`);
                    }
                    manifest.basePath = path.substring(0, path.lastIndexOf('/'));
                    apps.push(manifest);
                } catch (error) {
                    console.error(`Error loading plugin manifest from ${path}:`, error);
                    // In a real app, you might want to show a notification to the user
                    UI.Feedback.toast(`Błąd ładowania wtyczki: ${path}`, 'error');
                }
            }
            return apps;
        } catch (error) {
